...
-- ─── social_metrics ────────────────────────────────────────────
-- Actual columns: id, company_id, category, metric_name, metric_value,
--                 unit, year, gri_standard, source_origin, verified,
...
-- ─── roadmap_milestones ─────────────────────────────────────────
-- Actual columns: id, company_id, title, description, target_year,
--                 category, target_value, current_value, unit, status,
--                 sbti_aligned, gri_reference, created_at, updated_at
-- status CHECK: ('planned','in_progress','achieved','missed')
...
-- ─── advisory_sessions ─────────────────────────────────────────
-- Actual columns: id, user_id, persona, title, messages,
--                 created_at, updated_at
...
VALUES
  (
    ...
    '[
      {"role":"user","content":"我們公司的範疇三排放量如何計算？"},
      {"role":"assistant","content":"範疇三排放量依 GHG Protocol 分為 15 個類別。對製造業而言，最重要的通常是：上游 - 採購商品與服務 (Cat.1)、資本財 (Cat.2)；下游 - 已售商品使用 (Cat.11)。建議先進行重要性評估，聚焦前 3 大類別，通常佔範疇三排放量的 80% 以上。"}
    ]'::jsonb
  ),
  (
    'default', 'compliance', 'TCFD 氣候財務揭露準備',
    '[
      {"role":"user","content":"如何準備 TCFD 氣候相關財務揭露？"},
      {"role":"assistant","content":"TCFD 框架包含四大核心要素：1) 治理 - 董事會對氣候風險的監督；2) 策略 - 氣候風險與機會對業務的影響；3) 風險管理 - 識別、評估與管理流程；4) 指標與目標。建議先完成氣候情境分析（1.5°C 與 4°C 情境），再對應各部門的財務衝擊評估。"}
    ]'::jsonb
  ),
  (
    'default', 'harmony', 'ESG 員工參與文化建立',
    '[
      {"role":"user","content":"如何提升員工對 ESG 的參與感？"},
      {"role":"assistant","content":"ESG 文化建立需要由上而下的承諾與由下而上的參與。建議：1) 設立跨部門 ESG 大使計畫；2) 將個人 ESG 貢獻納入績效考核；3) 舉辦內部永續黑客松；4) 定期分享進展。讓永續不只是企業責任，更是個人價值實踐。"}
    ]'::jsonb
  ),
  (
    'default', 'innovation', '循環經濟商業價值諮詢',
    '[
      {"role":"user","content":"循環經濟如何為製造業創造商業價值？"},
      {"role":"assistant","content":"循環經濟在製造業有三大商業模式：1) 產品即服務 (PaaS)；2) 工業共生 - 與周邊企業交換廢棄物與副產品；3) 再製造 - 回收舊品翻新。預計 2030 年前循環經濟市場規模將達 4.5 兆美元。"}
    ]'::jsonb
  )
ON CONFLICT DO NOTHING;

-- ─── published_reports ─────────────────────────────────────────
-- Actual columns: id, company_id, title, year, framework,
--                 status, page_count, word_count, gri_coverage,
--                 zkp_verified, zkp_hash, created_at, updated_at

INSERT INTO public.published_reports
  (company_id, title, year, framework, status, page_count, gri_coverage, zkp_verified, zkp_hash)
VALUES
  (
    'default', '2023 永續報告書', 2023,
    ARRAY['GRI 2021', 'TCFD', 'SASB'],
    'published', 156, 78, TRUE,
    'sha256:a1b2c3d4e5f6789012345678901234567890abcdef'
  ),
  (
    'default', '2022 永續報告書', 2022,
    ARRAY['GRI 2021', 'TCFD'],
    'published', 128, 71, TRUE,
    'sha256:b2c3d4e5f6789012345678901234567890abcdef'
  ),
  (
    'default', '2024 永續報告書 (草稿)', 2024,
    ARRAY['GRI 2021', 'TCFD', 'SASB', 'ISSB S2'],
    'draft', 0, 45, FALSE,
    NULL
  ),
  (
    'default', '2024 中期進度報告', 2024,
    ARRAY['GRI 2021'],
    'published', 48, 35, TRUE,
    'sha256:c3d4e5f6789012345678901234567890abcdef'
  ),
  (
    'default', '碳中和路徑白皮書', 2024,
    ARRAY['TCFD', 'SBTi'],
    'published', 64, 20, TRUE,
    'sha256:d4e5f6789012345678901234567890abcdef'
  )
ON CONFLICT DO NOTHING;
