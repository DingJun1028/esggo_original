-- ============================================================
-- ESG GO | Omni_Terminal — Schema v6.0 Idempotent Migration
-- Safe to run multiple times — uses business key upserts
...
DO $$ BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'social_metrics_bk'
    AND conrelid = 'public.social_metrics'::regclass
  ) THEN
    ALTER TABLE public.social_metrics
    ADD CONSTRAINT social_metrics_bk UNIQUE (company_id, category, metric_name, year);
  END IF;
END $$;

DO $$ BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'governance_metrics_bk'
    AND conrelid = 'public.governance_metrics'::regclass
  ) THEN
    ALTER TABLE public.governance_metrics
    ADD CONSTRAINT governance_metrics_bk UNIQUE (company_id, category, metric_name, year);
  END IF;
END $$;

DO $$ BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'roadmap_milestones_bk'
    AND conrelid = 'public.roadmap_milestones'::regclass
  ) THEN
    ALTER TABLE public.roadmap_milestones
    ADD CONSTRAINT roadmap_milestones_bk UNIQUE (company_id, title, target_year);
  END IF;
END $$;

DO $$ BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'advisory_sessions_bk'
    AND conrelid = 'public.advisory_sessions'::regclass
  ) THEN
    ALTER TABLE public.advisory_sessions
    ADD CONSTRAINT advisory_sessions_bk UNIQUE (user_id, persona, title);
  END IF;
END $$;

DO $$ BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'published_reports_bk'
    AND conrelid = 'public.published_reports'::regclass
  ) THEN
    ALTER TABLE public.published_reports
    ADD CONSTRAINT published_reports_bk UNIQUE (company_id, title, year);
  END IF;
END $$;

-- ─── Clean Duplicate Rows (keep latest per business key) ────

DELETE FROM public.social_metrics a
USING public.social_metrics b
WHERE a.id < b.id
...
DELETE FROM public.governance_metrics a
USING public.governance_metrics b
WHERE a.id < b.id
  AND a.company_id = b.company_id
  AND a.category = b.category
  AND a.metric_name = b.metric_name
...
DELETE FROM public.roadmap_milestones a
USING public.roadmap_milestones b
WHERE a.id < b.id
  AND a.company_id = b.company_id
  AND a.title = b.title
  AND a.target_year = b.target_year;

DELETE FROM public.advisory_sessions a
USING public.advisory_sessions b
WHERE a.id < b.id
  AND a.user_id = b.user_id
  AND a.persona = b.persona
  AND a.title = b.title;

DELETE FROM public.published_reports a
USING public.published_reports b
WHERE a.id < b.id
  AND a.company_id = b.company_id
  AND a.title = b.title
  AND a.year = b.year;

-- ─── Upsert social_metrics ──────────────────────────────────
...
ON CONFLICT (company_id, category, metric_name, year)
DO UPDATE SET
  metric_value  = EXCLUDED.metric_value,
  unit          = EXCLUDED.unit,
  gri_standard  = EXCLUDED.gri_standard,
  source_origin = EXCLUDED.source_origin,
  verified      = EXCLUDED.verified,
  updated_at    = NOW();

-- ─── Upsert governance_metrics ──────────────────────────────
...
ON CONFLICT (company_id, title, target_year)
DO UPDATE SET
  ...
  sbti_aligned  = EXCLUDED.sbti_aligned,
  gri_reference = EXCLUDED.gri_reference,
  updated_at    = NOW();

-- ─── Upsert advisory_sessions ───────────────────────────────

INSERT INTO public.advisory_sessions (user_id, persona, title, messages)
VALUES
...
ON CONFLICT (user_id, persona, title)
DO UPDATE SET
  messages   = EXCLUDED.messages,
  updated_at = NOW();

-- ─── Upsert published_reports ───────────────────────────────

INSERT INTO public.published_reports
  (company_id, title, year, framework, status, page_count, gri_coverage, zkp_verified, zkp_hash)
VALUES
  ('default','2023 永續報告書',        2023,ARRAY['GRI 2021','TCFD','SASB'],          'published',156,78,TRUE, 'sha256:a1b2c3d4e5f678901234'),
  ('default','2022 永續報告書',        2022,ARRAY['GRI 2021','TCFD'],                 'published',128,71,TRUE, 'sha256:b2c3d4e5f678901234'),
  ('default','2024 永續報告書 (草稿)', 2024,ARRAY['GRI 2021','TCFD','SASB','ISSB S2'],'draft',    0,  45,FALSE,NULL),
  ('default','2024 中期進度報告',      2024,ARRAY['GRI 2021'],                         'published',48, 35,TRUE, 'sha256:c3d4e5f678901234'),
  ('default','碳中和路徑白皮書',       2024,ARRAY['TCFD','SBTi'],                      'published',64, 20,TRUE, 'sha256:d4e5f678901234')
ON CONFLICT (company_id, title, year)
DO UPDATE SET
  framework    = EXCLUDED.framework,
  status       = EXCLUDED.status,
  page_count   = EXCLUDED.page_count,
  gri_coverage = EXCLUDED.gri_coverage,
  zkp_verified = EXCLUDED.zkp_verified,
  zkp_hash     = EXCLUDED.zkp_hash,
  updated_at   = NOW();
