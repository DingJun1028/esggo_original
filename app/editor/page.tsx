...
// ── GRI Chapter Data (Master Production Set: 20+ Chapters) ─────────────────
interface Chapter {
...
const CHAPTERS: Chapter[] = [
  { id: 'intro', num: '00', title: '董事長聲明與永續策略', titleEn: 'Statement', gri: 'GRI 2-22', category: 'G', order: 0, estPages: 10 },
  { id: 'general', num: '01', title: '組織概況與治理架構', titleEn: 'General', gri: 'GRI 2-1 ~ 2-30', category: 'G', order: 1, estPages: 25 },
  { id: 'materiality', num: '02', title: '重大性議題分析流程', titleEn: 'Materiality', gri: 'GRI 3-1 ~ 3-3', category: 'G', order: 2, estPages: 15 },
  { id: 'economic', num: '03', title: '經濟績效與稅務揭露', titleEn: 'Economic', gri: 'GRI 201, 207', category: 'G', order: 3, estPages: 18 },
  { id: 'procurement', num: '04', title: '供應鏈治理與採購實務', titleEn: 'Procurement', gri: 'GRI 204', category: 'G', order: 4, estPages: 12 },
  { id: 'energy', num: '05', title: '能源管理與轉型佈署', titleEn: 'Energy', gri: 'GRI 302', category: 'E', order: 5, estPages: 25 },
  { id: 'water', num: '06', title: '水資源管理與循環利用', titleEn: 'Water', gri: 'GRI 303', category: 'E', order: 6, estPages: 20 },
  { id: 'emissions', num: '07', title: '溫室氣體排放 (Scope 1-3)', titleEn: 'Emissions', gri: 'GRI 305', category: 'E', order: 7, estPages: 40 },
  { id: 'waste', num: '08', title: '廢棄物減量與處理', titleEn: 'Waste', gri: 'GRI 306', category: 'E', order: 8, estPages: 18 },
  { id: 'labor', num: '09', title: '勞雇關係與幸福職場', titleEn: 'Labor', gri: 'GRI 401', category: 'S', order: 9, estPages: 22 },
  { id: 'ohs', num: '10', title: '職業安全與健康管理', titleEn: 'OHS', gri: 'GRI 403', category: 'S', order: 10, estPages: 25 },
...
export default function EditorPage() {
  ...
  const { 
    ...
    updateContent, updateChapterStatus, loading: memoryLoading 
    ...
    const progressTimer = setInterval(() => {
      setGenProgress(p => {
        ...
        const labels = [
          '正在構思深度大綱...',
          '執行 GRI 2021 指標對齊...',
          '專家模組：正在擴充 1000-3000 字...',
          '專家模組：正在擴充 3000-5000 字...',
          '執行數據一致性校準...'
        ];
    ...
    try {
      ...
      if (json.success) {
        ...
        Object.entries(fieldValues[chapter.id] || {}).forEach(([key, val]) => {
          if (val && !content.includes(val.toString())) {
             gaps.push({ field: key, expected: val.toString() });
          }
        });
        setDataGaps(gaps);
  ...
  const handleSeal = async () => {
    ...
    try {
      const component = await omniCore.sealComponent(
        `GRI_DOC_LEN:${(generatedContent[chapter.id] || '').length}`,
        `/reports/2026/${chapter.id}`,
        `[5T_INTEGRITY_PROTOCOL:SHA256]`
      );
  ...
  return (
    <div className="flex flex-col h-screen overflow-hidden bg-[#F8FAFC] text-slate-900 font-sans selection:bg-blue-500/10">
      ...
      <div className="flex-1 flex overflow-hidden">
        {/* Navigation Sidebar */}
        <aside className={cn("bg-white/60 backdrop-blur-2xl border-r border-slate-200 transition-all duration-500 flex flex-col z-20 shadow-xl", navCollapsed ? 'w-20' : 'w-72')}>
           <div className="px-6 py-5 flex items-center justify-between border-b border-slate-100">
              {!navCollapsed && <span className="text-[10px] font-black text-[#003262] uppercase tracking-[0.3em]">Module Index</span>}
              <button onClick={() => setNavCollapsed(!navCollapsed)} className="p-2 hover:bg-blue-50 rounded-xl transition-all ml-auto text-slate-400">
           ...
           <div className="flex-1 overflow-y-auto px-4 py-6 space-y-2 no-scrollbar">
              {CHAPTERS.map(c => (
                <button key={c.id} onClick={() => setSelectedChapterId(c.id)} className={cn(
                  "w-full flex items-center gap-4 p-3.5 rounded-2xl transition-all duration-300 group",
                  selectedChapterId === c.id ? 'bg-[#003262] text-white shadow-2xl scale-[1.02]' : 'text-slate-500 hover:bg-blue-50/50'
                )}>
                  <div className={cn("w-9 h-9 rounded-xl flex items-center justify-center font-black text-xs transition-all", selectedChapterId === c.id ? 'bg-white/20 text-white' : 'bg-slate-100 text-slate-400')}>{c.num}</div>
                  {!navCollapsed && (
           ...
           <div className="flex-1 overflow-y-auto p-10 flex flex-col xl:flex-row gap-10">
                 ...
                 <Card className="border-none shadow-premium bg-white/80 backdrop-blur-xl rounded-[2.5rem] overflow-hidden">
                    ...
                    <CardContent className="p-8 pt-0 space-y-6">
                       ...
                       <div className="pt-8 border-t border-slate-100 space-y-4">
                          <p className="text-[9px] font-black text-slate-400 uppercase tracking-[0.4em]">Expert Master Toolset</p>
              ...
              <div className="flex-1 flex flex-col">
                 <Card className="flex-1 border-none shadow-premium bg-white rounded-[3rem] overflow-hidden flex flex-col relative ring-1 ring-slate-100">
                    <div className="h-12 px-12 border-b border-slate-50 flex items-center justify-between bg-slate-50/30">
                       <div className="flex items-center gap-3">
                          <Type size={14} className="text-slate-400" />
                          <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">GRI Document Master Workspace</span>
                       </div>
                       {dataGaps.length > 0 && (
                         <div className="flex items-center gap-2 bg-red-50 px-3 py-1 rounded-full border border-red-100 animate-pulse">
                            <AlertTriangle size={10} className="text-red-500" />
                            <span className="text-[9px] font-black text-red-600 uppercase">Data_Mismatch_Detected</span>
                         </div>
                       )}
                    </div>
                    ...
                    <div className="flex-1 relative">
                       <textarea
                         ...
                         className="w-full h-full p-12 text-lg font-medium leading-[2.2] text-slate-700 outline-none resize-none bg-transparent"
                         placeholder="ESG 治理主權由您執筆，或啟動 AI 專家模組生成 250 頁 A4 等級的深度報告..."
                       />
                       
                       {generating && (
                         <div className="absolute inset-0 bg-white/80 backdrop-blur-xl flex items-center justify-center z-20">
                            <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="w-[400px] p-10 bg-[#003262] rounded-[3rem] text-white shadow-extreme relative overflow-hidden">
                               <div className="relative z-10 space-y-8">
                                  <div className="flex justify-between items-end">
                                     <div className="space-y-1">
                                        <p className="text-[10px] font-black text-blue-300 uppercase tracking-[0.3em]">AI Recursive Expansion</p>
                                        <h4 className="text-xl font-black">{genProgress.label}</h4>
                                     </div>
                                     <span className="text-2xl font-black font-mono text-[#FDB515]">{Math.round((genProgress.step / genProgress.total) * 100)}%</span>
                                  </div>
                                  <div className="h-2 w-full bg-white/10 rounded-full overflow-hidden">
                                     <motion.div className="h-full bg-gradient-to-r from-blue-400 to-[#FDB515]" initial={{ width: 0 }} animate={{ width: `${(genProgress.step / genProgress.total) * 100}%` }} transition={{ duration: 1 }} />
                                  </div>
                                  <div className="flex items-center gap-4 text-blue-200/60">
                                     <RefreshCw size={16} className="animate-spin" />
                                     <p className="text-[10px] font-bold uppercase tracking-widest">目標：5000 字專家級深度撰寫</p>
                                  </div>
                               </div>
                               <Zap size={180} className="absolute -bottom-10 -right-10 text-white/5 rotate-12" />
                            </motion.div>
                         </div>
                       )}
                    </div>
