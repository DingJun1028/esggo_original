...
export default function EditorPage() {
  ...
  const handleAutoPopulate = async () => {
    setGenerating(true);
    showToast(`Hermes 正在檢索金庫中與 ${chapter.gri} 相關的憑證...`, 'info');
    try {
      const { data: nexus } = await supabase.rpc('get_gri_nexus', { p_gri_tag: chapter.gri });
      const vaultArtifact = nexus?.find((n: any) => n.artifact_type === 'VAULT');
      if (!vaultArtifact) { showToast('金庫中查無此章節的相關憑證', 'error'); return; }
      const { extractMetricsFromEvidence } = await import('../../lib/hermes-gateway');
      const { metrics } = await extractMetricsFromEvidence(vaultArtifact.artifact_id);
      metrics.forEach(m => {
        if (m.gri === chapter.gri || chapter.gri.includes(m.gri)) {
           const targetField = chapter.fields?.find(f => f.gri === m.gri) || chapter.fields?.[0];
           if (targetField) updateFieldValue(chapter.id, targetField.id, m.value.toString());
        }
      });
      showToast('Hermes 已完成數據自動填報', 'success');
    } catch (e) { showToast('智慧填報引擎故障', 'error'); } finally { setGenerating(false); }
  };
  ...
  return (
    <div className="flex flex-col h-screen overflow-hidden bg-[#F8FAFC] text-slate-900 font-sans selection:bg-blue-500/10">
      <header className="h-16 bg-white/80 backdrop-blur-2xl border-b border-slate-200 px-8 flex items-center justify-between z-30 shadow-sm">
        ...
        <div className="flex items-center gap-4"><div className="flex items-center gap-4 px-6 py-2 bg-slate-50 rounded-2xl border border-slate-100 mr-4"><div className="text-right"><p className="text-[8px] font-black text-slate-400 uppercase tracking-widest">Global Integrity</p><p className="text-xs font-black text-[#003262] font-mono">T5_CERTIFIED</p></div><BrandStatusDot status="active" pulse size="md" /></div><Button variant="primary" size="default" onClick={handleSeal} isLoading={sealing} disabled={isSealed} className="rounded-2xl px-8 shadow-xl shadow-blue-500/10"><Lock size={18} className="mr-2" /> {isSealed ? '已封印' : '5T 封印'}</Button></div>
      </header>
